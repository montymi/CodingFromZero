...
** Key Commands
While reading below, if (argument) is in the definition of a command, any nested text will be the possible text to add after the command.
If tab complete (marked as (TC)) is in definiction of a command, then the (argument) of the command can be filled or listed using the ~<TAB>~ key
