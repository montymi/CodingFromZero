...
# NOTE: running print(f"") allows variables to be passed as seen below
...
#+begin_src python :results output
# The `def` statement must fall BEFORE the function is called otherwise Python won't know what to call
# START OF FUNCTION
def greet(name):
"""
